...
//si on a pas id du produit qu'on a passer 
if(!isset($_GET['id'])){
    header("Location: afficher.php");
}
//n'est pas vide et une variable numérique
...
<?php foreach ($Produits as $produit): ?>
    ...
    <textarea class="form-control" name="ref"required><?= $produit->ref ?></textarea>
  </div>

  <div class="mb-3">
    
    <label for="exampleInputPassword1" class="form-label">Taille</label><br>
      <input type="text" class="form-control" name="taille" value="<?= $produit->taille ?>"  required> <br><br>

    </div> 

  <div class="mb-3">
    <label for="exampleInputPassword1" class="form-label">couleur</label><br>
    <input type="text" class="form-control" name="couleur" value="<?= $produit->couleur ?>"  required> <br><br>
  </div>
    ...
    <textarea class="form-control" name="nbp" required> <?= $produit->nbp ?></textarea>
  </div>
...
//vérifier les données 
    if(isset($_POST['valider']))
    {
        if(isset($_POST['image']) AND isset($_POST['nom']) AND isset($_POST['nbp']) AND isset($_POST['prix']) AND isset($_POST['desc']) AND isset($_POST['couleur'])AND isset($_POST['taille']) AND isset($_POST['ref']))
        {
            ...
            try 
            {
              modifier($image, $ref, $nom, $couleur, $taille, $nbp, $prix, $desc, $id);
              header('Location: afficher.php');
            } 
            catch (Exception $e) 
            {
